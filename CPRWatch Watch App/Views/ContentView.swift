...
struct ContentView: View {
    ...
    var body: some View {
        ScrollView { VStack(spacing: 10) { isRunning ? AnyView(activeView) : AnyView(setupView) }.padding(.horizontal, 8) }
            .sheet(isPresented: $showSafety) { safetyView }
            .onReceive(Timer.publish(every: 0.05, on: .main, in: .common).autoconnect()) { now in update(now) }
    }
    ...
    private var setupView: some View {
        VStack(spacing: 12) {
            ...
            Picker("Cadence", selection: $cadence) { ForEach(CadencePreset.allCases) { Text("\($0.rawValue) BPM").tag($0) } }
            Picker("Mode", selection: $mode) { ForEach(CPRMode.allCases) { Text($0.rawValue).tag($0) } }
            Button("Start CPR", action: start).buttonStyle(.borderedProminent)
    ...
    private var activeView: some View {
        VStack(spacing: 8) {
            ...
            HStack { Button(isPaused ? "Resume" : "Pause", action: togglePause).buttonStyle(.bordered); Button("Stop", role: .destructive, action: stop).buttonStyle(.bordered) }
    ...
    private var safetyView: some View {
        Text("Call local emergency services and follow dispatcher and AED instructions. CPRWatch only provides timing assistance.").font(.footnote).padding()
    }
    ...
    private func update(_ now: Date) {
        ...
        if !isPaused, state.beat != lastBeat { if state.phase.isCompression { WKInterfaceDevice.current().play(.click) }; lastBeat = state.beat }
    }

    private func start() { startedAt = Date(); accumulatedPause = 0; state = .idle; lastBeat = -1; isPaused = false; isRunning = true; WKInterfaceDevice.current().play(.start) }

    private func togglePause() {
        if isPaused { if let pausedAt { accumulatedPause += Date().timeIntervalSince(pausedAt) }; self.pausedAt = nil; isPaused = false; WKInterfaceDevice.current().play(.start) }
        else { pausedAt = Date(); isPaused = true; WKInterfaceDevice.current().play(.stop) }
    }

    private func stop() { isRunning = false; isPaused = false; startedAt = nil; pausedAt = nil; state = .idle; WKInterfaceDevice.current().play(.stop) }
...
private extension CPRPhase {
    var isCompression: Bool { if case .compressions = self { return true }; return false }
}
